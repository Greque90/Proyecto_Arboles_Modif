...
def sup_ha_por_barrio(barrios_df):
    """
    Superficie por nombre_barrio, sumando filas duplicadas.
    barrios_de_la_ciudad.csv tiene más de una fila para algunos barrios (polígonos en
    varias partes con el mismo nombre); indexar directo por nombre_barrio deja índices
    repetidos y cualquier .get()/merge por nombre devuelve más de un valor en vez de uno.
    """
    return barrios_df.groupby("nombre_barrio")["sup_ha"].sum()
...
def dibujar_choropleta(barrios_geo, valores, titulo, etiqueta_barra, cmap_nombre="YlOrRd"):
    ...
    if valores_validos.index.duplicated().any():
        # Salvaguarda: si 'valores' viene con nombres de barrio repetidos, .get() por nombre
        # devolvería más de un valor y rompe el coloreado. Promediamos por nombre.
        valores_validos = valores_validos.groupby(level=0).mean()
...
with tabs[1]:
    ...
    _sup_ha = sup_ha_por_barrio(barrios)
    _densidad_barrio = (
...
with tabs[6]:
    ...
    resumen = resumen.merge(
        sup_ha_por_barrio(barrios).rename("sup_ha"), left_index=True, right_index=True, how="left"
    )
